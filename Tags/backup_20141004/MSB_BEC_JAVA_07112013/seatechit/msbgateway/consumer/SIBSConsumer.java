...
public class SIBSConsumer extends BaseConsumer {
	...
	private void getCustomerInfoOffline() {
		try {
			...
			if (retCustomerInfo != null) {
				...
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, CoreBankUtils.convertCB2EBDateYYYYMMdd(retCustomerInfo.getBirth_date()), "birth_date");
	...
	private void getCustomerAccounts() {
		try {
			...
			// TODO 26/06/2012
			String online = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xpathInput, "is_online"));
			if (cif_no.length() > 0) {
	...
	private void getRelCustomerAccounts() {
		try {
			...
			String online = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputSummary, "is_online"));
			...
			for (int i = 0; i < iTotalRecord; i++) {
				xpathInput = "/message/body/list[@name='records']/entity[@name='" + (i) + "']/field[@name='%s']";
				cif_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xpathInput, "cif_no"));
				acct_type = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xpathInput, "acct_type"));
				acct_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xpathInput, "acct_no"));
				acct_ccy = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xpathInput, "acct_ccy"));
				listOfCif[i] = cif_no;
			}
			if (iTotalRecord == 1) {
				if (cif_no.length() > 0) {
	 ...
	 * @param acct_ccy
	 */
	private void getAccountInfoOffline(String acct_no, String acct_type, String acct_ccy, String online) {
		try {
			String strRequestMsg = "";
			String strResponseMsg = "";
			...
			this.hostMessageLogInfo.setSend_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
			try {
				tempAccount = acctFacade.getAccountInfoByAccountNo(acct_no, acct_type, online);
			} catch (Exception ex) {
			}
			this.hostMessageLogInfo.setReceive_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
			if (tempAccount == null) {
			...
			if (retErrorInfo.getErrorCode() == ERR_SYSTEM_OK) {
				...
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, tempAccount.getEstablish_date(), "establish_date");
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, tempAccount.getAvailable_date(), "available_date");
				...
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, tempAccount.getDate_acct_opened(), "date_acct_opened");
				...
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, tempAccount.getTerm_code(), "term_code");
				...
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, tempAccount.getIssued_date(), "issued_date");
				ElementUtils.setXMLElementValue(outMessage, xpathOutput, tempAccount.getMaturity_date(), "maturity_date");
	 ...
	 * 
	 */
	private void getListOfCustomerAccountOffline(String[] cif_no, String online, int numOfCif) {
		try {
			AccountInfoFacade acctFacade = new AccountInfoFacade();
			...
			try {
				if (numOfCif == 1) {
					if (cif_no[0].length() > 0) {
						arrAccountInfo = acctFacade.getAccountInfoByCif(cif_no[0], online);
					} else {

					}
				} else if (numOfCif > 1) {
					arrAccountInfo = acctFacade.getRelAccountInfoByCif(cif_no, numOfCif, online);
				}
			} catch (Exception ex) {
			}
			this.hostMessageLogInfo.setReceive_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
			...
			if (arrAccountInfo == null || arrAccountInfo.size() == 0) {
				...
				for (int i = 0; i < arrAccountInfo.size(); i++) {
					...
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAccount.getEstablish_date(), "establish_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAccount.getAvailable_date(), "available_date");
					...
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAccount.getDate_acct_opened(), "date_acct_opened");
					...
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAccount.getTerm_code(), "term_code");
					...
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAccount.getIssued_date(), "issued_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAccount.getMaturity_date(), "maturity_date");
				...
				if (retMessage != null) {
					if (retMessage.getErrCode() == null) {
						...
						if (retMessage.getErrCode().equals("DDM0118")) {
							this.retErrorInfo.setErrorCode(Global.ERR_UDF_CASHFLOW_ACCOUNT_PRIMARY_HIGHERLEVEL);
							this.retErrorInfo.setErrorMessage(retMessage.getDescription());
							this.retErrorInfo.setErrorType(MESSAGE_TYPE_MBASE);
						} else if (retMessage.getErrCode().equals("DDM0116")) {
							this.retErrorInfo.setErrorCode(Global.ERR_UDF_CASHFLOW_ACCOUNT_ALREADY_SECONDARY);
							this.retErrorInfo.setErrorMessage(retMessage.getDescription());
							this.retErrorInfo.setErrorType(MESSAGE_TYPE_MBASE);
						} else {
							this.retErrorInfo.setErrorCode(Global.ERR_MBASE_START);
					...
					this.retErrorInfo.setErrorMessage("ERR_SYSTEM_UNKNOWN_ERROR");
					this.retErrorInfo.setErrorType(MESSAGE_TYPE_MBASE);
				}
				...
				if (retMessage != null) {
					if (retMessage.getErrCode() == null) {
						...
						if (retMessage.getErrCode().equals("MB28131")) {
							this.retErrorInfo.setErrorCode(Global.ERR_UDF_CASHFLOW_RECORD_NOTFOUND);
							this.retErrorInfo.setErrorMessage(retMessage.getDescription());
							this.retErrorInfo.setErrorType(MESSAGE_TYPE_MBASE);
						} else {
							this.retErrorInfo.setErrorCode(Global.ERR_MBASE_START);
	...
	private void createFDAccount() {
		try {
			...
			String fd_acct_no = as400Port.getAccount(sub_fd_branch_no, "01", "05");
			String strRequestMsg = "";
			String strResponseMsg = "";
			if (fd_acct_no != null && fd_acct_no.length() > 0) {
				...
				if (retMessage != null) {
					if (retMessage.getErrCode() == null) {
						...
						outMessage.evaluateXPath(String.format(xmlOuputPattern, AppUtils.now(Global.DEF_FORMAT_CREATE_DATE_TIME), "create_time"));
					}
					strResponseMsg = AppUtils.convertArrayToString(retMessage.getArrString());
				} else {
	...
	private void createFDReceipt() {
		try {
			...
			this.hostMessageLogInfo.setSend_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
			String receipt_no = as400Port.getFDReceiptAccount(sub_fd_branch_no, currRefCode, fd_group);
			String strRequestMsg = "";
			String strResponseMsg = "";
			if (receipt_no != null) {
				this.hostMessageLogInfo.setSend_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
				try {
					...
					strRequestMsg = buildCreateFDReceipt(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getHost_tran_date(), fd_branch_no, hostMessageLogInfo.getTeller_id(),
							hostMessageLogInfo.getApprover_id(), hostMessageLogInfo.getHost_tran_sn(), hostMessageLogInfo.getHost_tran_code(), autoRenew, fd_acct_no, receipt_no,
							fd_acct_ccy, fd_product_code, deposit_amt, remark, interest_receive_acct, interest_receive_acct);
					retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), strRequestMsg);
					...
					if (retMessage != null) {
						...
						if (retMessage.getErrCode() == null) {
							...
							outMessage.evaluateXPath(String.format(xpathOutput, AppUtils.now(Global.DEF_FORMAT_CREATE_DATE_TIME), "create_time"));
						} else {
				...
				if (retMessage != null) {
					...
					if (retMessage.getErrCode() == null) {
						...
						if (retMessage.getErrCode().equals("102")) {
							this.retErrorInfo.setErrorCode(Global.ERR_UDF_ACTIVE_ATM_CARD_NOTFOUND);
							this.retErrorInfo.setErrorMessage(retMessage.getDescription());
						} else if (retMessage.getErrCode().equals("120")) {
							this.retErrorInfo.setErrorCode(Global.ERR_UDF_ACTIVE_ATM_CARD_NOTREACTIVE);
							this.retErrorInfo.setErrorMessage(retMessage.getDescription());
						} else {
							this.retErrorInfo.setErrorCode(Global.ERR_MBASE_START);
							this.retErrorInfo.setErrorMessage(retMessage.getDescription());
	...
	private void fundTransfer() {
		try {
			...
			for (int i = 0; i < listOfTransfer.size(); i++) {
				...
				String rollout_acct_name = AppUtils.convertToVietnamesNoSign(ElementUtils.getXMLElementInStringValue(inMessage,
						String.format(xmlInputPattern_2, "rollout_acct_name")));
				...
				String bnfc_branch_name = AppUtils
						.convertToVietnamesNoSign(ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern_2, "bnfc_branch_name")));
				...
				String orgFee = CoreBankUtils.roundCBNumberic(ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern_2, "fee")));
				...
				if (tempErrorInfo.getErrorCode() == ERR_SYSTEM_OK) {
					if (is_outward_transfer.equals("N")) {// internal-bank
						...
						if (rollout_acct_type.equals(Global.COREBBANK_CA_ACCOUNT_TYPE)) {
							...
							if (bnfc_acct_type.equals(Global.COREBBANK_CA_ACCOUNT_TYPE)) {
							...
							} else if (bnfc_acct_type.equals(Global.COREBBANK_SA_ACCOUNT_TYPE)) {
								if (bnfc_branch_no.equals(rollout_branch_no)) {
							...
							else if (bnfc_acct_type.equals(Global.COREBBANK_FD_RECEIPT_ACCOUNT_TYPE)) {
						...
						if (tempErrorInfo.getErrorCode() == ERR_SYSTEM_OK) {
							String vatFee = String.valueOf(new BigDecimal(orgFee).divide(new BigDecimal(Global.BASED_VAT_RATE), 0, RoundingMode.HALF_UP));
							String serviceFee = String.valueOf(new BigDecimal(orgFee).subtract(new BigDecimal(vatFee)).longValue());

							this.hostMessageLogInfo.setSend_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
							rebuildTransactionInfo(refTranCode, rollout_branch_no);
							try {
								if (refTranCode.equals(SIBS_018) || refTranCode.equals(SIBS_019)) {// CA->CA
									this.retMessage = as400Port.transferFromCASAToCASA(hostMessageLogInfo.getRef_channel(), rollout_branch_no, bnfc_acct_no, amt, bnfc_acct_ccy,
											bnfc_sell_rate, rollout_acct_no, amt, rollout_acct_ccy, rollout_buy_rate, hostMessageLogInfo.getApprover_id(), remark,
											hostMessageLogInfo.getHost_tran_sn(), hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getHost_tran_date(),
											hostMessageLogInfo.getHost_tran_code(), vatFee, serviceFee);
								} else if (refTranCode.equals(SIBS_020) || refTranCode.equals(SIBS_021)) {// CA->SA
									amt = CoreBankUtils.convertEB2CBNumeric(amt);
									serviceFee = CoreBankUtils.convertEB2CBNumeric(serviceFee);
									vatFee = CoreBankUtils.convertEB2CBNumeric(vatFee);
									String totalFee = CoreBankUtils.convertEB2CBNumeric(orgFee);
									String sendMSG = buildCASAMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getHost_tran_date(), rollout_branch_no,
											hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getApprover_id(), hostMessageLogInfo.getHost_tran_sn(), rollout_acct_no,
											bnfc_acct_no, amt, serviceFee, vatFee, totalFee, remark, hostMessageLogInfo.getHost_tran_code());
									this.retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), sendMSG);
								} else if (refTranCode.equals(SIBS_037) || refTranCode.equals(SIBS_038)) {// SA->CA
									amt = CoreBankUtils.convertEB2CBNumeric(amt);
									serviceFee = CoreBankUtils.convertEB2CBNumeric(serviceFee);
									vatFee = CoreBankUtils.convertEB2CBNumeric(vatFee);
									String totalFee = CoreBankUtils.convertEB2CBNumeric(orgFee);
									String sendMSG = buildSACAMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getHost_tran_date(), rollout_branch_no,
											hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getApprover_id(), hostMessageLogInfo.getHost_tran_sn(), rollout_acct_no,
											bnfc_acct_no, amt, serviceFee, vatFee, totalFee, remark, hostMessageLogInfo.getHost_tran_code());
									this.retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), sendMSG);
								} else if (refTranCode.equals(SIBS_039) || refTranCode.equals(SIBS_040)) {// SA->SA
									amt = CoreBankUtils.convertEB2CBNumeric(amt);
									serviceFee = CoreBankUtils.convertEB2CBNumeric(serviceFee);
									vatFee = CoreBankUtils.convertEB2CBNumeric(vatFee);
									String totalFee = CoreBankUtils.convertEB2CBNumeric(orgFee);
									String sendMSG = buildSASAMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getHost_tran_date(), rollout_branch_no,
											hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getApprover_id(), hostMessageLogInfo.getHost_tran_sn(), rollout_acct_no,
											bnfc_acct_no, amt, serviceFee, vatFee, totalFee, remark, hostMessageLogInfo.getHost_tran_code());
									this.retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), sendMSG);
								}// ngocnv
									...
									String sendMSG = buildTransferCAtoFDMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getTeller_id(),
											hostMessageLogInfo.getHost_tran_sn(), hostMessageLogInfo.getHost_tran_code(), rollout_acct_no, bnfc_acct_no, amt, totalFee, remark,
											hostMessageLogInfo.getHost_tran_date(), rollout_branch_no, vatFee);

									this.retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), sendMSG);
									
									//check errCode
									if(retMessage!= null &&  "79".equals(retMessage.getErrCode())) {
										
										String reSendMSG = reBuildTransferCAtoFDMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getTeller_id(),
												hostMessageLogInfo.getHost_tran_sn(), hostMessageLogInfo.getHost_tran_code(), rollout_acct_no, bnfc_acct_no, amt, totalFee, remark,
												hostMessageLogInfo.getHost_tran_date(), rollout_branch_no, vatFee);
											
										this.retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), reSendMSG);
									}
						...
						if (bnfc_acct_name.length() > 70 || bnfc_acct_name.length() < 0) {
							tempErrorInfo.setErrorCode(Global.ERR_UDF_TRANSFER_RECEIVE_NAME_NOT_VALID_OR_MISSING);
							tempErrorInfo.setErrorMessage("ERR_UDF_TRANSFER_RECEIVE_NAME_NOT_VALID_OR_MISSING");
						...
						if (tempErrorInfo.getErrorCode() == ERR_SYSTEM_OK) {
							...
							try {
								...
								String seqNumber = sysParamFacade.getRMSequence();
								String reversalDate = CoreBankUtils.convertCBDateReversal(hostMessageLogInfo.getHost_tran_date());
								rm_ref_no = rollout_branch_no + "2" + reversalDate + AppUtils.padLeft(seqNumber, 4, '0');

								String sendMessage = buildOL2Message(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getHost_tran_date(), rollout_branch_no,
										hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getApprover_id(), hostMessageLogInfo.getHost_tran_sn(), rm_ref_no, GLAccount, cif_no,
										rollout_acct_name, cert_code, rollout_acct_no, toAccount, toName, toId, toAddress, toIdIssueDate, toIdIssuePlace, amt, serviceFee, vatFee,
							...
							hostRequestMessage = "[channel:" + hostMessageLogInfo.getRef_channel() + "," + "branchCode:" + rollout_branch_no + "," + "teller:"
									...
									+ hostMessageLogInfo.getHost_tran_sn() + "," + "rm_ref_no:" + rm_ref_no + "," + "tran_date:" + hostMessageLogInfo.getHost_tran_date() + ","
									+ "rollout_acct_no:" + rollout_acct_no + "," + "amt:" + amt + "," + "cif_no:" + cif_no + "," + "glAccount:" + GLAccount + "," + "vatFee:"
									+ vatFee + "," + "serviceFee:" + serviceFee + "," + "remarks:" + remark + "," + "cif_acct_name:" + cif_acct_name + "," + "cert_code:"
									+ cert_code + "," + "toAccount:" + toAccount + "," + "toName:" + toName + "," + "toId:" + toId + "," + "toAddress:" + toAddress + ","
									+ "toIdIssueDate:" + toIdIssueDate + "," + "toIdIssuePlace:" + toIdIssuePlace + "," + "transCode:" + hostMessageLogInfo.getHost_tran_code()
									+ "]";
							this.hostMessageLogInfo.setReceive_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
						}
					}
	...
	private void settlementFDAccount() {
		try {
			...
			if (retErrorInfo.getErrorCode() == 0) { // Check amt
				...
				fee = String.valueOf(Math.round(Double.parseDouble(fee) - dblVATCharge));
				// --end-----
				...
				try {
					...
					if (retMessage != null) {
						...
						strResponseMsg = AppUtils.convertArrayToString(this.retMessage.getArrString());
					} else {
						// TODO: kienvt bo xung them
						retErrorInfo.setErrorCode(ERR_SYSTEM_UNKNOWN_ERROR);
						retErrorInfo.setErrorMessage("ERR_SYSTEM_UNKNOWN_ERROR");
	 ...
	 * @return
	 */
	private String buildCreateFDReceipt(String hostIP, String transactionDate, String branchCode, String tellerId, String approverId, String refId, String tranCode,
			String autoreNew, String fd_account_no, String fd_receipt_no, String fd_receipt_ccy, String product_code, String amount, String remark, String principal_account,
	...
	private String buildOL2Message(String hostIP, String transactionDate, String branchCode, String tellerId, String approverId, String refId, String rm_ref_no, String glAccount,
		...
		// remark
		fromName = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(fromName, 41));
		toId = AppUtils.subStringBySpecLength(toId, 196);
		glAccount = AppUtils.subStringBySpecLength(glAccount, 19);
		toName = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(toName, 70));
		bank_id = AppUtils.subStringBySpecLength(bank_id, 43);
		bank_name = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(bank_name, 40));
		toAccount = AppUtils.subStringBySpecLength(toAccount, 20);
		toAddress = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(toAddress, 113));
		toIdIssueDate = AppUtils.subStringBySpecLength(toIdIssueDate, 144);
		remarks = "(RmNo: " + rm_ref_no + ") " + remarks;
		remarks = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(remarks, 180));
		...
		toName1 = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(toName1, 40));
		...
		toName2 = AppUtils.convertToVietnamesNoSign(AppUtils.subStringBySpecLength(toName2, 30));
		...
		message += "RM        " + "C                            "
				...
				+ AppUtils.padRight(bank_name, 40, ' ') + AppUtils.padRight(toAddress.toUpperCase(), 113, ' ') + "0000000000000000000      "
				+ AppUtils.padLeft(String.valueOf(amount), 54, '0') // amt
				+ "            " + toIdIssueDate + AppUtils.padRight("", 87, ' ') + AppUtils.padRight(toName2.toUpperCase(), 30, ' ') + AppUtils.padRight("", 27, ' ') + "|";
	 ...
	 * @return
	 */
	private String buildCASAMessage(String hostIP, String transactionDate, String branchCode, String tellerId, String approverId, String refId, String fromAccount,
			String toAccount, String amount, String serviceFee, String vatFee, String totalFee, String remarks, String tranCode) {
		...
		String message = "*LINX|"
				...
				+ fromAccount + "||" + totalFee + "|||" + serviceFee + "|" + vatFee + "||||||||" + amount + "|||5|||4|4||||||||1||||||||||||||||" + remarks + "||||" + amount
				+ "|000|000||||||||3|1|1|||||||";
	 ...
	 * @return
	 */
	private String buildSACAMessage(String hostIP, String transactionDate, String branchCode, String tellerId, String approverId, String refId, String fromAccount,
			String toAccount, String amount, String serviceFee, String vatFee, String totalFee, String remarks, String tranCode) {
		...
		String message = "*LINX|"
				...
				+ fromAccount + "|" + totalFee + "||||" + serviceFee + "|" + vatFee + "|000|||||||||5||||4|4|4|||||||||||||||||||||||" + remarks + "||||" + amount
				+ "|000|000||||||||3|1|1|||||||";
	 ...
	 * @return
	 */
	private String buildSASAMessage(String hostIP, String transactionDate, String branchCode, String tellerId, String approverId, String refId, String fromAccount,
			String toAccount, String amount, String serviceFee, String vatFee, String totalFee, String remarks, String tranCode) {
	...
	private void settingAFT() {
		try {
			...
			String xmlInputPattern = "/message/body/field[@name='%s']";
			String cif_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "cif_no"));
			String rollout_acct_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "rollout_acct_no"));
			String rollout_acct_type = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "rollout_acct_type"));
			String bnfc_acct_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "bnfc_acct_no"));
			String bnfc_acct_type = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "bnfc_acct_type"));
			String freq = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "freq"));
			String freq_code = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "freq_code"));
			String started_date = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "started_date"));
			String expired_date = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "expired_date"));
			String aft_type = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "aft_type"));
			String aft_prod = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "aft_prod"));
			String amount = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "amount"));
			String fee = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "fee"));
			String remark = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "remark"));
			...
			try {
				...
				strRequestMsg = buildSettingAFTMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getHost_tran_code(), rollout_acct_no,
						rollout_acct_type, bnfc_acct_no, bnfc_acct_type, aft_type, started_date, expired_date, amount, freq, freq_code, fee, aft_prod);
	...
	private void destroyAFT() {
		try {
			...
			String aft_prod = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "aft_prod"));
			String amount = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "amount"));
			String fee = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "aft_fee"));
			String sequence = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "sequence"));
			...
			try {
				amount = CoreBankUtils.convertEB2CBNumeric(amount);
				fee = CoreBankUtils.convertEB2CBNumeric(fee);
				strRequestMsg = buildLockAFTMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getHost_tran_code(), rollout_acct_no,
						rollout_acct_type, sequence);
				...
				if (retMessage != null) {
					...
					if (retMessage.getErrCode() != null) {
						...
						this.hostMessageLogInfo.setSend_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
						strRequestMsg = buildDestroyAFTMessage(hostMessageLogInfo.getWs_ip(), hostMessageLogInfo.getTeller_id(), hostMessageLogInfo.getHost_tran_code(),
								rollout_acct_no, rollout_acct_type, sequence, bnfc_acct_no, bnfc_acct_type, aft_type, started_date, expired_date, amount, freq, freq_code, fee,
								aft_prod);
						retMessage = as400Port.hostMessageSending(hostMessageLogInfo.getRef_channel(), strRequestMsg);
						if (retMessage != null) {
							strResponseMsg = AppUtils.convertArrayToString(retMessage.getArrString());
	...
	private void queryAFT() {
		...
		getQueryAFT(cif_no, rollout_acct_no, rollout_acct_type, bnfc_acct_no, bnfc_acct_type, aft_prod);
	...
	private void queryFDReceipt() {
		...
		String xmlInputPattern = "/message/body/field[@name='%s']";
		String cif_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "cif_no"));
		String group_acct = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "group_acct"));
		String receipt_no = ElementUtils.getXMLElementInStringValue(inMessage, String.format(xmlInputPattern, "receipt_no"));
	 ...
	 * @return
	 */
	private String buildSettingAFTMessage(String hostIP, String tellerId, String tranCode, String accountNo, String typeAccountNo, String accountCo, String typeAccountCo,
			String codeAFT, String startDate, String endDate, String moneyTransfer, String TS, String typeTS, String moneyFee, String AFT) {
		String message = "*LINX|"
				+ hostIP
	 ...
	 * @return
	 */
	private String buildDestroyAFTMessage(String hostIP, String tellerId, String tranCode, String accountNo, String typeAccountNo, String seq, String accountCo,
			String typeAccountCo, String codeAFT, String startDate, String endDate, String moneyTransfer, String TS, String typeTS, String moneyFee, String AFT) {
		String message = "*LINX|"
				+ hostIP
				+ "||||213||0200|*DSP|MBSD|*LINX||||01|||BBMBSDDMNTFNC|||||1|10|"
				+ tellerId
	 ...
	 * @return
	 */
	private String buildLockAFTMessage(String hostIP, String tellerId, String tranCode, String accountNo, String typeAccountNo, String seq) {
		String message = "*LINX|"
				+ hostIP
	 ...
	 * @return
	 */
	private String buildTransferCAtoFDMessage(String hostIP, String tellerId, String refId, String tranCode, String CA, String FD, String amount, String fee, String remark,
			String date, String branch, String vatFee) {
		String message = "*LINX|"
				+ hostIP
				+ "| | | |213|42|0200| |ABCS|*LINX| | | | | | |BBHTLMONEYFNC| | | | | | |"
				+ tellerId
				...
				+ amount
				+ "|||||10000000|10000000|10000000|10000000|||VND||||||||||||VND|VND|VND|VND||||3||||||||||||||1|4|4|2|||||||||||||4|4|4||||||1|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
				+ CA + "||" + fee + "|||" + fee + "|" + vatFee + "|000|||||||" + amount + "|||5|||4|4|4|||||||2||||||||||||||||" + remark + "|||||000|000|||||||||1|1|||||||";

		return message;
	}
	
	@SuppressWarnings("unused")
	private String reBuildTransferCAtoFDMessage(String hostIP, String tellerId, String refId, String tranCode, String CA, String FD, String amount, String fee, String remark,
			String date, String branch, String vatFee) {
		String message = "*LINX|"
				+ hostIP
				+ "| | | |213|42|0200| |ABCS|*LINX| | | | | | |BBHTLMONEYFNC| | | | | | |"
	...
	private void getQueryAFT(String cif_no, String rollout_acct_no, String rollout_acct_type, String bnfc_acct_no, String bnfc_acct_type, String aft_prod) {
		try {
			...
			this.hostMessageLogInfo.setSend_time(AppUtils.now(Global.DEF_FORMAT_LOGMSG_DATE_TIME));
			try {
				arrAftInfo = aftFacade.getAftInfos(cif_no, rollout_acct_no, bnfc_acct_no, rollout_acct_type, bnfc_acct_type, aft_prod);
			} catch (Exception ex) {
			}
			...
			if (arrAftInfo == null || arrAftInfo.size() == 0) {
				...
				for (int i = 0; i < arrAftInfo.size(); i++) {
					...
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getStatus(), "status");
					ElementUtils
							.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.correctAccountNumber(tempAft.getRollout_Acct_No()), "rollout_acct_no");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getRollout_acct_ccy(), "rollout_acct_ccy");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getRollout_acct_type(), "rollout_acct_type");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.correctAccountNumber(tempAft.getBnfc_Acct_No()), "bnfc_acct_no");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getBnfc_acct_ccy(), "bnfc_acct_ccy");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getBnfc_acct_type(), "bnfc_acct_type");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempAft.getStarted_Date()), "started_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempAft.getExpired_Date()), "expired_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempAft.getNext_Transfer_Date()),
							"next_transfer_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempAft.getLast_Transfer_Date()),
							"last_transfer_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.correctBranchCode(tempAft.getAmount()), "amount");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getAft_Fee(), "aft_fee");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getFreq_No(), "freq_no");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getFreq_Code(), "freq_code");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getDebit_Seq(), "debit_seq");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempAft.getEntered_date()), "entered_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempAft.getBnfc_acct_prod_type(), "bnfc_acct_prod_type");
	...
	private void getQueryFDReceipt(String cif_no, String group_acct, String receipt_no) {
		try {
			...
			if (arrFDInfo == null || arrFDInfo.size() == 0) {
				...
				for (int i = 0; i < arrFDInfo.size(); i++) {
					...
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getGroup_Acct(), "group_acct");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.correctAccountNumber(tempFD.getReceipt_No()), "receipt_no");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getReceipt_Product_Code(), "receipt_product_code");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getInterest_Rate(), "interest_rate");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), AppUtils.XMLReplaceSpecChar(tempFD.getOriginal_Balance()), "original_balance");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempFD.getIssue_Date()), "issue_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), CoreBankUtils.convertDdMMyyToYyyyMMyy(tempFD.getMaturity_Date()), "maturity_date");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getAccurated_Interest(), "accurated_interest");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getCur_Type(), "cur_type");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getCd_Term(), "cd_term");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getAvailable_Balance(), "available_balance");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getHold(), "hold");
					ElementUtils.setXMLElementValue(outMessage, xpathOutput, String.valueOf(i), tempFD.getStatus(), "status");
