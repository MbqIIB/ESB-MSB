...
public class HostProcessor {
	...
	public static Messages sendMessage(int socketNumber, String[] messageArray, String channel, String hostIP) {
		...
		try {
			...
			String arrStr = AppUtils.convertArrayToString(messageArray);
			Logger.getLogger(HostProcessor.class).info("Input Mess:" + arrStr);
	...
	private static Messages sendMBASE(int socketNumber, String[] message, String seq, String hostIP) {
		...
		try {
			...
			// Put data to Host
			Logger.getLogger(HostProcessor.class).info("SIBS putdata");
			sk.getOutputStream().write(btRollback);

			// Get response from Host
			Logger.getLogger(HostProcessor.class).info("SIBS End putdata");
			byte[] btResponse = (byte[]) DSPPackager.loadDSPMessage(sk.getInputStream());
			
			byte[] encoded = Base64.encodeBase64(btResponse);
			String outsend = new String(encoded);
			Logger.getLogger(HostProcessor.class).info("SIBS check responsed byte array:" + outsend);
